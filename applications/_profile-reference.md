...
- **ASRP — реальный титул, реальное название, реальная структура.** (⚠️ Отменяет предыдущее решение от 2026-07-22 — см. ниже.) **Титул: "Co-Founder & CTO"** — использовать его как есть, не заменять на "Lead Engineer" или другие эвфемизмы. **Полное название: "Advanced Scientific Research Projects"** — не "independent AI research lab". **Структура: US- and KZ-based holding**, не generic "research lab". Эталонная формулировка для Experience-заголовка: **"Co-Founder & CTO — ASRP (Advanced Scientific Research Projects, US- and KZ-based holding)"**.
  - **Отменённое решение (было в силе 2026-07-22 – 2026-07-23):** ранее избегали "Co-Founder & CTO" из-за риска, что рекрутер прочитает это как "flight risk"/overqualified/не будет слушаться. Пользователь явно отменил это 2026-07-23: **"Вернуть реальный титул Co-Founder & CTO"** — писать прямо везде, включая CV.
  - Уже сгенерированные CV/письма (Interloom, ChatRevenue.ai Principal, Nova Digital), где стоит "Lead Engineer — ASRP (independent AI research lab)", нужно обновить на новую формулировку.
...
- **Self-описание в Summary — backend + AI, не "full-stack generalist".** (Утверждено 2026-07-23.) Не писать "Full-stack engineer... across frontend, backend, and DevOps" — это неточно и переоценивает DevOps-компетенцию. Верная формулировка: backend- и AI-ориентированный инженер, который может подключиться к фронтенду при необходимости и имеет опыт работы с облачными провайдерами (AWS/GCP/Azure), но **не DevOps-инженер** — не заявлять DevOps как отдельную профильную компетенцию наравне с backend/AI. Эталон (Summary): *"Backend- and AI-focused engineer with 10+ years of experience, comfortable working across the full stack and with cloud infrastructure (AWS/GCP/Azure) when needed. The last 2-3 years have focused on production LLM/multi-agent systems and graph-based knowledge representation, including architecting and owning a multi-tenant agent platform end to end."* В Skills-гриде категория тоже переименована: **"Cloud / Infra"**, не "Cloud / DevOps" (та же причина — не завышать DevOps как заявленную специализацию; список инструментов внутри категории — Docker, Terraform, CI/CD, AWS/GCP/Azure — не меняется, это реальные инструменты, вопрос только в названии категории/self-identity).
- **ASRP — НЕ упоминать в Summary вообще** (утверждено 2026-07-23, после обсуждения). Summary фокусируется только на техническом профиле (backend+AI, 10+ лет, LLM/agents) — упоминание ASRP остаётся только в Experience-секции (там уже есть "Co-Founder & CTO — ASRP (Advanced Scientific Research Projects, US- and KZ-based holding)" с датами и деталями) — этого достаточно, отдельное дублирование в Summary не нужно.
- **ASRP — не "текущая работа, которую брошу ради найма", а действующая исследовательская практика/бизнес, который я не планирую закрывать.** (Утверждено 2026-07-23; формулировка-позиционирование ниже теперь живёт в Experience, не в Summary — см. правило выше про удаление ASRP из Summary.) Позиционировать в Experience-заголовке/описании ASRP как ongoing: R&D ASRP (agent-архитектуры, knowledge graphs) напрямую подпитывает качество delivery на клиентских engagement'ах — не "я работаю в ASRP, пока не найду что-то получше", а "у меня есть исследовательская база, которая делает мою работу на вас лучше". Формулировка-эталон для cover letter (где уместно упомянуть ongoing-природу помимо Experience-заголовка): *"I run engineering at ASRP (Advanced Scientific Research Projects) as Co-Founder & CTO — an ongoing US- and KZ-based holding, not something I'm looking to wind down — its R&D on agent architectures and knowledge graphs directly feeds the quality of what I deliver on client work."*
- **Тип занятости — открыт к full-time, contract, freelance; не настроен именно на "устройство в штат".** (Утверждено 2026-07-23, ответ пользователя: *"я не уверен что я к кому-то иду работать в штат, скорее либо фриланс либо через существующие юр лица: KZ/US"*.) Во всех CV/cover letter добавлять явную фразу об открытости к разным форматам занятости, а не только full-time employee:
  - **CV Summary:** финальное предложение Summary — "EU citizen, Berlin-based, open to full-time, contract, or freelance engagements." (ASRP из Summary убран — см. правило выше, эта фраза идёт сразу после технического описания профиля).
...
- **Cover letter — тоже нужен `cover-letter.pdf`, не только `.md`** (утверждено 2026-07-23). Тот же паттерн, что и CV: собирать `cover-letter.html` на основе мастер-шаблона [`_cover-letter-template.html`](_cover-letter-template.html) (простой деловой формат письма — имя+контакты в шапке, `hr`, затем параграфы; НЕ формат резюме — без skills-грида/секций), конвертировать markdown-разметку `cover-letter.md` в HTML вручную (`**bold**` → `<strong>`, `[text](url)` → `<a>`, пустая строка → `<p>`), рендерить через weasyprint, проверить (`pdfinfo` + визуально через `pdftoppm`), затем удалить `.html`-файл — оставить в папке только `cover-letter.md` (референс) + `cover-letter.pdf` (на отправку). Cover letter не обязан влезать в 1 страницу так же строго, как CV (это письмо, не резюме) — 1 страница ожидаема при текущей длине текста, но не критично, если станет чуть длиннее.
- **Hi-letter — PDF НЕ нужен.** Это короткое первое сообщение (email/чат/форма на сайте) — отправляется как обычный текст, не вложением. Только `hi-letter.md`.
- **Контактная строка (CV header + cover letter header + sign-off) — включает email.** (Утверждено 2026-07-23.) Формат: `kapustinomm@gmail.com · Berlin, Germany · github.com/Kapustin2000/ · linkedin.com/in/mykhailo-kapustin-55885612a` — email идёт первым. В cover letter sign-off — email отдельной строкой после GitHub-ссылки.
